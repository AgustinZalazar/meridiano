  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        {params.code ? (
          <View style={s.codeRow}><Text style={s.codeText}>{params.code}</Text></View>
        ) : null}

        <View style={s.field}>
          <Text style={s.fieldLabel}>NOMBRE</Text>
          <TextInput style={s.input} value={name} onChangeText={setName}
            placeholder="Ej. Instalaciones Eléctricas" placeholderTextColor={colors.faint}
        ...
        <View style={s.field}>
          ...
          <TextInput style={s.input} value={contractor} onChangeText={setContractor}
            placeholder="Ej. Electro Sur S.A." placeholderTextColor={colors.faint}
            selectionColor={colors.arena} returnKeyType="next" />
        </View>
        ...
        <View style={s.field}>
          ...
          <TextInput style={s.input} value={code} onChangeText={setCode}
            placeholder="Ej. RB-001" placeholderTextColor={colors.faint}
            selectionColor={colors.arena} autoCapitalize="characters" returnKeyType="done" />
        </View>
        ...
        {rubroId && (
          <TouchableOpacity
            style={s.hitosRow}
            onPress={() => router.push({ pathname: '/hitos/[rubroId]', params: { rubroId, rubroName: params.name ?? '', projectId: projectId ?? '' } })}
            activeOpacity={0.75}
          >
            <View style={s.hitosRowLeft}>
              <Feather name="check-square" size={16} color={colors.crema} />
              <Text style={s.hitosRowText}>Hitos del rubro</Text>
            </View>
...
const s = StyleSheet.create({
  ...
  hitosRow: {
    ...
    justifyContent: 'space-between', backgroundColor: colors.panel,
    borderRadius: 16, paddingHorizontal: spacing.md, paddingVertical: 16,
  },
  hitosRowLeft: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
  hitosRowText: { fontFamily: fonts.archivo.semibold, fontSize: 15, color: colors.crema },
  ...
  ctaBlock: { paddingHorizontal: spacing.xl, gap: spacing.md },
